...
struct hkConsoleOutputLine
{
	struct Asm : Xbyak::CodeGenerator {
		Asm() {
			...
			L(label1);
			dq(RelocAddr<decltype(&Hook)>(0x0286ED55 + 5).getUIntPtr()); // 0x028D8945
		}
	};

	static void Apply() {
		g_Trampoline.write5Branch(RelocAddr<uintptr_t>(0x0286ED55).getUIntPtr(), (uintptr_t)&Hook); // 0x028D8945
	}
...
extern "C"
{
	__declspec(dllexport) SFSEPluginVersionData SFSEPlugin_Version = {
		SFSEPluginVersionData::kVersion, 1, "Console Hooks", "stonegdi", 0, 0, { RUNTIME_VERSION_1_10_31, 0 }, 0, 0, 0,
	};
